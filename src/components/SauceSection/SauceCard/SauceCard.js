...
import styles from './SauceCard.module.css';
import cartIco from '../../../assets/fonts/ico-cart.svg';
import {addGoodsAC, plusToSumAC} from "../../../redux/cart-reducer";
import {useDispatch} from "react-redux";

export const SauceCard = ({data}) => {

    const {id, name, price, photo} = data;

    const {zoom, titleContainer, title, txt, cartButton, container, hr, ico, cardAndPrice} = styles;

    const dispatch = useDispatch();

    const onAddCart = (id, name, price, count = 1) => {
        dispatch(addGoodsAC({
            id,
            name,
            price,
            ...
            <div>
                <p className={cardAndPrice}>
                    ...
                    <button className={cartButton}
                            type={'button'}
                            onClick={() => onAddCart(id, name, price, 1)}
                    >
